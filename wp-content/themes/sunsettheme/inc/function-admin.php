...
function sunset_add_admin_page() {
	...
	add_menu_page( 'Sunset Theme Options', 'Sunset', 'manage_options', 'alecaddd_sunset', 'sunset_theme_create_page', get_template_directory_uri() .
	 '/img/sunset-icon.png', 110 );
...
function sunset_custom_settings() {
	...
	register_setting( 'sunset-settings-group', 'twitter_handler', 'sunset_sainitize_twitter_handler' );
	...
	add_settings_field( 'post-formats', 'Post Formates', 'sunset_post_formats', 'alecaddd_sunset_theme', 'sunset-theme-options' );
	...
	add_settings_field( 'custom-css', 'Insert Your Custom CSS', 'sunset_custom_css_callback', 'alecaddd_sunset_css', '' );
...
function sunset_sidebar_profile() {
	...
	if( empty($picture ) ){
		echo '<input type="button" class="button button-secondy" value="Upload Profile Picture" id="upload-button"> <input type="hidden" id="profile-piture" name="profile_picture" value=""  /> ';
	} else {
	echo '<input type="button" class="button button-secondy" value="Replace Profile Picture" id="upload-button"> <input type="hidden" id="profile-piture" name="profile_picture" value="'.$picture.'" /><input type="button" class="button button-secondy" value="Remove" id="remove-picture"> ';
	}
}
...
function sunset_sidebar_description() {
	...
	echo '<input type="text" name="user_description" value="'.$description.'" placeholder="Write Description Here" />';
}

function sunset_sidebar_twitter() {
	$twitter = esc_attr( get_option( 'twitter_handler' ) );
	echo '<input type="text" name="twitter_handler" value="'.$twitter.'" placeholder="Twitter handler" /> <p class="description">Input Twitter account without @ symbol.</p>';
}
...
function sunset_sainitize_twitter_handler( $input ) {
